...
# DQualley, 11/11/19, Modified code to complete assignment 6
# ------------------------------------------------------------------------ #
...
class FileProcessor:
    ...
    @staticmethod #created function to reload data from file
    def ReloadDataFromFile(file_name, list_of_rows):
        """ this function reloads data from the original file
        """
        if (strYesOrNo.lower() == 'y'):
            list_of_rows.clear()  # Added to fix bug 1.1.2030
            FileProcessor.ReadFileDataToList(file_name, list_of_rows)  # Replace the current list data with file data
            IO.ShowCurrentItemsInList(list_of_rows)  # Show current data in the list/table
        else:
